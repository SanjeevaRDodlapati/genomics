...
def group_files(files):
    ...
    for file in files:
        parts = file.rsplit('_', 1)  # Splitting only at the 2nd to last underscore
        if len(parts) != 2:
...
def get_clinvar_enrich(cells, pred1_path, inPath, level, row_labels, cosmic_df, thr):
    ...
    cosmic_df : pd.DataFrame
        DataFrame containing COSMIC noncoding variant information with columns "chrom" and "pos".
    thr : float
    ...
    for cell in cells:
        try:
            ...
            # Merge with COSMIC data on 'chrom' and 'pos'
            merged_df1 = pd.merge(cosmic_df, data1, on=["chrom", "pos"], how="inner")
            merged_df2 = pd.merge(cosmic_df, data2, on=["chrom", "pos"], how="inner")
...
def main(group, thr, limit):
    """
    Main function to run COSMIC enrichment analysis.
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="COSMIC Enrichment Analysis")
